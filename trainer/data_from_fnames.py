...
class DataFromFNames(Dataset):
    ...
    Args:
        fname_lists (list): A list of filenames or tuple of filenames, e.g.
            ['image_001.png', ...] or
            [('pair_image_001_0.png', 'pair_image_001_1.png'), ...].
        shapes (tuple): Shapes of data, e.g. [256, 256, 3] or
            [[256, 256, 3], [1]].
        random (bool): Read from `fname_lists` randomly (default to False).
    ...
    def __init__(self, fname_lists, shapes, random=False, random_crop=False,
                 fn_preprocess=None, dtypes=tf.float32,
                 enqueue_size=32, queue_size=256, nthreads=8,
                 return_fnames=False, filetype='image'):
        self.fname_lists_ = self.process_fname_lists(fname_lists)
        self.file_length = len(self.fname_lists_)
        self.random = random
        ...
        if isinstance(shapes[0], list):
            ...
            self.shapes = [shapes] * len(self.fname_lists_[0])
        if isinstance(dtypes, list):
            self.dtypes = dtypes
        else:
            self.dtypes = [dtypes] * len(self.fname_lists_[0])
    ...
    def process_fname_lists(self, fnamelist):
    ...
    def create_queue(self, shared_name=None, name=None):
        ...
        self._queue = data_flow_ops.FIFOQueue(capacity=capacity, dtypes=self.dtypes, shapes=self.shapes,
    ...
    def next_batch(self):
        ...
        for _ in range(self.enqueue_size):
            ...
            while error:
                error = False
                if random:
                    filenames = random.choice(self.fname_lists_)
                else:
                    with READER_LOCK:
                        filenames = self.fname_lists_[self.index]
